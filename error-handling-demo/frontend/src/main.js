...
function renderIndex() {
    ...
    .map(([key, category]) => `
      <article class="category-card">
        <div class="category-title-row">
          ...
          <div>
            <span class="category-number">Category ${category.order}</span>
            <h2>${category.title}</h2>
            <span class="error-chip">${category.errorType}</span>
          </div>
        </div>
        <div class="category-actions">
          <a class="page-link without-link" href="${hrefFor({ technique: key, mode: 'without' })}">Without Error Handling</a>
          <a class="page-link handled-link" href="${hrefFor({ technique: key, mode: 'handled' })}">With Error Handling</a>
        </div>
      </article>
    `).join('')
  ...
  app.innerHTML = `
    ...
    <main class="page-shell">
      ...
      <section class="category-grid" aria-label="Error handling categories">
        ${cards}
      </section>
    </main>
  `
...
function renderApplicationPage(key, currentMode) {
  ...
  if (!category || !['without', 'handled'].includes(currentMode)) {
    renderIndex()
    return
  }

  const handled = currentMode === 'handled'
  const otherMode = handled ? 'without' : 'handled'
  const otherLabel = handled ? 'Without Error Handling' : 'With Error Handling'

  app.innerHTML = `
    ${pageHeader()}
    <main class="page-shell">
      <nav class="page-nav">
        <a href="${hrefFor()}">← Error Handling Categories</a>
        <a class="switch-link" href="${hrefFor({ technique: key, mode: otherMode })}">${otherLabel}</a>
      </nav>

      <section class="application-card">
        <div class="application-heading">
          <div>
            <span class="category-number">${category.errorType}</span>
            <h2>${category.title}</h2>
          </div>
          <span class="mode-badge ${handled ? 'handled' : 'without'}">${handled ? 'With Error Handling' : 'Without Error Handling'}</span>
        </div>

        <form id="bookForm" class="book-form" novalidate>
          <label>
            Book Title
            <input id="title" name="title" value="${escapeHtml(category.payload.title)}" />
            <small id="titleError" class="field-error"></small>
          </label>

          <label>
            Accession Number
            <input id="accessionNumber" name="accessionNumber" value="${escapeHtml(category.payload.accessionNumber)}" />
            <small id="accessionNumberError" class="field-error"></small>
          </label>

          <label>
            Author
            <input id="author" name="author" value="${escapeHtml(category.payload.author)}" />
          </label>
  ...
  attachApplicationHandler(category, currentMode)
...
function friendlyMessage(body, status) {
  ...
  if (status === 400) return 'The request contains invalid data.'
  if (status === 401) return 'Your session has expired. Please sign in again.'
  if (status === 403) return 'You do not have permission to perform this operation.'
  if (status === 404) return 'The requested record was not found.'
  if (status === 409) return 'This record conflicts with an existing record.'
...
function attachInputResetHandlers() {
  ...
  for (const id of ['title', 'accessionNumber', 'author']) {
    const input = document.querySelector(`#${id}`)
    if (!input) continue
...
function attachApplicationHandler(category, currentMode) {
  const form = document.querySelector('#bookForm')
  const statusArea = document.querySelector('#statusArea')
  const saveButton = document.querySelector('#saveButton')
  const apiUrl = category.apiUrl ?? DEFAULT_API_URL
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (currentMode === 'handled' && !validateClient(payload)) {
      ...
      statusArea.textContent = 'Please correct the highlighted fields.'
      return
    }
    ...
    if (currentMode === 'handled') {
      ...
      statusArea.innerHTML = '<span class="spinner" aria-hidden="true"></span> Saving...'
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (body.field && showFieldError(body.field, friendlyMessage(body, response.status))) {
          statusArea.className = 'status-area error-message'
          statusArea.textContent = 'The Book was not saved. Correct the highlighted field.'
        } else {
          statusArea.className = 'status-area error-message'
      ...
      statusArea.textContent = `Book saved successfully. Record ID: ${body.id}.`
    } catch (error) {
      ...
      if (currentMode === 'without') {
        ...
        statusArea.className = 'status-area error-message'
        statusArea.textContent = 'Unable to connect to the server. Please try again.'
      }
    } finally {
      if (currentMode === 'handled') {
...
if (categoryKey && mode) {
  renderApplicationPage(categoryKey, mode)
} else {
  renderIndex()
}
